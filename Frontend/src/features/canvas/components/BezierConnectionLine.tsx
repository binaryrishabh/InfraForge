...
export function BezierConnectionLine({
  ...
  // Anchor per axis so lines emerge from the card border they actually
  // leave/enter by, instead of a single min-dimension offset.
  ...
  const socketRing = isSelected ? "#5B8CFF" : "#3A465C";
  const socketCore = isSelected ? "#5B8CFF" : "#677185";
  ...
  return (
    <g>
      ...
      {/* Sockets gripping both line ends (fallback canvas; the unified
          canvas renders its sockets as DOM ports on the card edges). */}
      {showSockets && (
        <g pointerEvents="none">
          <circle cx={x1} cy={y1} r={4.5} fill="#0B0E14" stroke={socketRing} strokeWidth={1.5} />
          <circle cx={x1} cy={y1} r={1.5} fill={socketCore} />
          <circle cx={x2} cy={y2} r={4.5} fill="#0B0E14" stroke={socketRing} strokeWidth={1.5} />
          <circle cx={x2} cy={y2} r={1.5} fill={socketCore} />
        </g>
      )}
