...
import { createInitialState, tick, applyManualScale, reconcileTopology } from "@shared/simulation/engine";
...
describe("golden scenarios — topology & readiness", () => {
test("L2: readiness gate rejects an isolated node", () => {
const result = validateDeploymentReadiness(
[{ id: "vm-1", type: RESOURCE_TYPES.VirtualMachine, x: 0, y: 0 }],
[]
);
...
test("T1: reconcileTopology preserves runtime while rerouting a live edit", () => {
let state = createInitialState("golden-t1", SAMPLE_ARCHITECTURE.resources, SAMPLE_ARCHITECTURE.connectionLines, makeProfile({ targetThroughput: 1_000_000 }), 42);
for (let i = 0; i < 10; i++) state = tick(state, {}).state;

// Inject a crash on vm-1 plus a crash on cache-1 (which is about to be deleted)
state = {
...state,
activeChaos: [
...state.activeChaos,
{ chaosType: DeploymentChaosNames.Crash, resourceId: "vm-1", durationTicks: 30, remainingTicks: 30 },
{ chaosType: DeploymentChaosNames.Crash, resourceId: "cache-1", durationTicks: 30, remainingTicks: 30 }
]
};
const before = state;

// Live edit: remove cache-1 entirely and add one extra VM behind lb-1
const newResources = [
...SAMPLE_ARCHITECTURE.resources.filter(r => r.id !== "cache-1"),
{ id: "vm-3", type: RESOURCE_TYPES.VirtualMachine, x: 288, y: 768 }
];
const newConnections = [
...SAMPLE_ARCHITECTURE.connectionLines.filter(c => c.sourceId !== "cache-1" && c.targetId !== "cache-1"),
{ id: "c-new", sourceId: "lb-1", targetId: "vm-3", sourceType: "Load Balancer", targetType: "Virtual Machine", port: 80 }
];

const after = reconcileTopology(before, newResources, newConnections);

// vm-1 runtime preserved — same crashed cpu reading as before the edit
expect(after.metrics["vm-1"]!.cpu).toBe(before.metrics["vm-1"]!.cpu);

// vm-1 crash preserved; chaos targeting the deleted cache-1 dropped
expect(after.activeChaos.length).toBe(1);
expect(after.activeChaos[0]!.resourceId).toBe("vm-1");
expect(after.activeChaos[0]!.chaosType).toBe(DeploymentChaosNames.Crash);

// Pool re-anchored: the new VM joined lb-1's base replicas
expect(after.pools["lb-1"]!.baseVmIds).toContain("vm-3");

// cache-1 removed and db-1 left unreachable — it becomes idle
expect(after.reachable).not.toContain("db-1");
expect(after.idle).toContain("db-1");
});
});